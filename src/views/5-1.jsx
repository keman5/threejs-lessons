  ...
  useEffect(() => {
    const $ = {
      ...
      datGui () {
        ...
        gui.add(_this.mesh.material, 'gradientMap', ['none', 'three', 'five',]).onChange(val => {
          if (val === 'none') {
            _this.mesh.material.gradientMap = null;
          } else if (val === 'three') {
            _this.threeToneTexture.dispose(); // 从内存中销毁纹理对象

            const threeToneTexture = _this.textureLoader.load('/src/assets/textures/threeTone.jpg');

            threeToneTexture.magFilter = THREE.NearestFilter;
            _this.mesh.material.gradientMap = threeToneTexture;

          } else {
            _this.fiveToneTexture.magFilter = THREE.NearestFilter;
            _this.mesh.material.gradientMap = _this.fiveToneTexture;
